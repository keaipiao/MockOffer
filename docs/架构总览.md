...
> 全局核心实体与关系；字段级细节在各功能 `设计.md` §2.3。

| 实体 | 职责 | 关键关系 |
|---|---|---|
| `users` | 账号（邮箱 / GitHub 身份、无密码） | 1—1 `user_profiles`；1—N `user_identities` |
| `user_identities` | 登录身份（邮箱 / GitHub，预留手机 / 微信） | 属于 `users`，`(provider, provider_uid)` 部分唯一（仅未软删行） |
...
- **ID 策略（全局）**：`users.id` 自增（社区场景可展示「用户编号」）；其余业务表统一雪花 ID（BIGINT、分布式、不暴露业务量）。
- **逻辑删除（全局）**：所有业务表带 `deleted_at` 软删、查询默认过滤；唯一约束用部分索引（`WHERE deleted_at IS NULL`）；物理删除交后续回收站 + 超期清理。
- **会话（全局）**：短期 access（带 `jti`）+ 长期 refresh（存 Redis 可吊销、轮换）；登出 / 解绑 / 禁用即时吊销（删 refresh + access jti 入黑名单）。

---

## 6. 环境与配置（dev / prod）

- **环境变量单一来源**：全项目环境变量集中在唯一一份 `deploy/.env`，前端 / 后端 / 各服务都从它读；**禁止在 `backend` / `frontend` 等子目录另建 `.env`**。缺变量去这份补、同步 `.env.example`，不新建文件。
